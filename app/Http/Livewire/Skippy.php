...
class Skippy extends Component
{
    ...
    public function addToQueue($trackID)
    {
        ...
        if ($songLookup !== null)
        {
        $this->emit('addedToQueue', $songLookup['name']);
        } else {
        $this->emit('requirePremium');
        }
    ...
    public function voteToSkip(Request $request)
    {
        ...
        {
        $this->saved = true;
        $this->emit('votedToSkip', $queue[$this->skiptrack[0]]);
        }
        }
    ...
    public function skipService()
    {
        ...
        */
         Log::info('SkipService called! - '. now());
         //$this->viewQueue();
         //$this->reset(viewQueue());
        return $this->test += 50;
